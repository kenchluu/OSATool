...
namespace OSATool
{
    public partial class Form_TableListEdit : Form
    {

        static string[] sheettablelist = new string[50];


        public Form_TableListEdit()
        {
            InitializeComponent();
            ...
            try
            {
                ...
                Int32 listcount = 0;
                if (GetWBProperty(objBook, "tableedit_listcount") != null)
                {
                    listcount = Convert.ToInt16(GetWBProperty(objBook, "tableedit_listcount"));
                    //MessageBox.Show(listcount.ToString());
                    if (listcount > 0)
                    {
                        for (Int32 kk = 0; kk < listcount; kk++)
                        {
                            string sheettablename = GetWBProperty(objBook, "tableedit_" + kk.ToString());
        ...
        private void Bt_Update_Click(object sender, EventArgs e)
        {
            ...
            if (this.dataGridView1.RowCount > 1)
            {
                for (Int32 kk = 0; kk < this.dataGridView1.RowCount; kk++)
                {
                    if (this.dataGridView1[0, kk].Value != null)
                    {
                        ...
                        {
                            SetWBProperty(objBook, "tableedit_" + listcount.ToString(), this.dataGridView1[0, kk].Value.ToString());
            ...
            if (listcount > 0)
            {
                SetWBProperty(objBook, "tableedit_listcount", listcount.ToString());
            }
            else
            {
                DelWBProperty(objBook, "tableedit_listcount");
            }
        ...
        {
            DelWBProperty(wb, "tableedit_listcount");
            for (Int32 i = 1; i < listcount; i++)
            {
                {
                    DelWBProperty(wb, "tableedit_" + i.ToString());
                }
