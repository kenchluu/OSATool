...
namespace OSATool
{
    public partial class Form_TableListView : Form
    {

        static string[] sheettablelist = new string[50];


        public Form_TableListView()
        {
            InitializeComponent();
            ...
            try
            {
                ...
                Int32 listcount = 0;
                if (GetWBProperty(objBook, "tableview_listcount") != null)
                {
                    listcount = Convert.ToInt16(GetWBProperty(objBook, "tableview_listcount"));
                    //MessageBox.Show(listcount.ToString());
                    if (listcount > 0)
                    {
                        for (Int32 kk = 0; kk < listcount; kk++)
                        {
                            string sheettablename = GetWBProperty(objBook, "tableview_" + kk.ToString());
                            if (sheettablename != null)
                            {
                                if (cbc.Items.Contains(sheettablename)) AddOutputRow(sheettablename);
                            }
                        }
                    }
        ...
        private void Bt_Update_Click(object sender, EventArgs e)
        {
            ...
            if (this.dataGridView1.RowCount > 1)
            {
                for (Int32 kk = 0; kk < this.dataGridView1.RowCount; kk++)
                {
                    if (this.dataGridView1[0, kk].Value != null)
                    {
                        ...
                        {
                            SetWBProperty(objBook, "tableview_" + listcount.ToString(), this.dataGridView1[0, kk].Value.ToString());
            ...
            if (listcount > 0)
            {
                SetWBProperty(objBook, "tableview_listcount", listcount.ToString());
            }
            else
            {
                DelWBProperty(objBook, "tableview_listcount");
            }
        ...
        void sheettablelist_clear(Excel.Workbook wb, Int32 listcount)
        {
            DelWBProperty(wb, "tableview_listcount");
            for (Int32 i = 1; i < listcount; i++)
            {
                {
                    DelWBProperty(wb, "tableview_" + i.ToString());
                }
