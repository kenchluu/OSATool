...
namespace OSATool
{
    public partial class Form_MacroWS : Form
    {

        Excel.Workbook wb = null;
        Excel.Worksheet ws = null;
        static string ChkStartWSMacroRepeat = null;
        static string ChkEndWSMacroRepeat = null;
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            ChkStartWSMacroRepeat = GetProperty(ws, "StartWSMacroRepeat");
            ChkEndWSMacroRepeat = GetProperty(ws, "EndWSMacroRepeat");

            if (ChkStartWSMacroRepeat != null)
            {
                this.chk_RepeatWSMacroBegin.Checked = true;

            }
            else
            {
                this.chk_RepeatWSMacroBegin.Checked = false;
            }

            if (ChkEndWSMacroRepeat != null)
            {
                this.chk_RepeatWSMacroEnd.Checked = true;

            }
            else
            {
                this.chk_RepeatWSMacroEnd.Checked = false;
            }

            this.dataGridView_Macro1.Rows.Clear();
            ...
            if (macrolistnames.Count > 0)
            {
                ...
                Int32 kk = 1;
                while (GetProperty(ws, "macrostart" + kk.ToString()) != null)
                {
                    string macroname = GetProperty(ws, "macrostart" + kk.ToString());
                    if (cbc_Macro.Items.Contains(macroname)) AddOutputRow_Macro1(macroname);
                    kk++;
                }
            }

            this.dataGridView_Macro2.Rows.Clear();
            macrolistnames = GetMacroList();
            if (macrolistnames.Count > 0)
            {
                DataGridViewComboBoxColumn cbc_Macro = (DataGridViewComboBoxColumn)this.dataGridView_Macro2.Columns[0];
                for (Int32 j = 0; j < macrolistnames.Count; j++)
                {
                    cbc_Macro.Items.Add(macrolistnames[j]);
                }
                ...
                Int32 kk = 1;
                while (GetProperty(ws, "macroend" + kk.ToString()) != null)
                {
                    string macroname = GetProperty(ws, "macroend" + kk.ToString());
                    if (cbc_Macro.Items.Contains(macroname)) AddOutputRow_Macro2(macroname);
                    kk++;
                }
            }
        }
        ...
        {          
            ...
            if (this.chk_RepeatWSMacroBegin.Checked)
            {
                SetProperty(ws, "StartWSMacroRepeat", "TRUE");
            }
            else
            {
                DelProperty(ws, "StartWSMacroRepeat");
            }


            Int32 jj = 1;
            while (GetProperty(ws, "macrostart" + jj.ToString()) != null)
            {
                DelProperty(ws, "macrostart" + jj.ToString());
                jj++;
            }
            ...
            if (this.dataGridView_Macro1.RowCount > 1)
            {
                for (Int32 kk = 0; kk < this.dataGridView_Macro1.RowCount; kk++)
                {
                    if (this.dataGridView_Macro1[0, kk].Value != null)
                    {
                        ...
                        {
                            SetProperty(ws, "macrostart" + listcount.ToString(), this.dataGridView_Macro1[0, kk].Value.ToString());
            ...
            if (this.chk_RepeatWSMacroEnd.Checked)
            {
                SetProperty(ws, "EndWSMacroRepeat", "TRUE");
            }
            else
            {
                DelProperty(ws, "EndWSMacroRepeat");
            }

            jj = 1;
            while (GetProperty(ws, "macroend" + jj.ToString()) != null)
            {
                DelProperty(ws, "macroend" + jj.ToString());
                jj++;
            }

            listcount = 1;
            if (this.dataGridView_Macro2.RowCount > 1)
            {
                for (Int32 kk = 0; kk < this.dataGridView_Macro2.RowCount; kk++)
                {
                    if (this.dataGridView_Macro2[0, kk].Value != null)
                    {
                        if (this.dataGridView_Macro2[0, kk].Value.ToString() != String.Empty)
                        {
                            SetProperty(ws, "macroend" + listcount.ToString(), this.dataGridView_Macro2[0, kk].Value.ToString());
                            listcount = listcount + 1;
                        }
                    }
        ...
        void AddOutputRow_Macro2(string comment1)
        {
            DataGridViewRow row = (DataGridViewRow)this.dataGridView_Macro2.Rows[0].Clone();
            row.Cells[0].Value = comment1;
            this.dataGridView_Macro2.Rows.Add(row);

            Int32 rowindex1 = this.dataGridView_Macro2.Rows.Count - 1;
            this.dataGridView_Macro2.Rows[rowindex1].Selected = true;
            this.dataGridView_Macro2.FirstDisplayedScrollingRowIndex = rowindex1;
            
        }

        private void Macro2_insertRowAbove_Click(object sender, EventArgs e)
        {
            try
            {
                Int32 rowIndex = this.dataGridView_Macro2.CurrentCell.RowIndex;
                this.dataGridView_Macro2.Rows.Insert(rowIndex, 1);
                this.dataGridView_Macro2.ClearSelection();
                //totalrownumber = this.dataGridView1.Rows.Count;
            }
            catch { }
        }

        private void Macro2_insertRowBelow_Click(object sender, EventArgs e)
        {
            try
            {
                Int32 rowIndex = this.dataGridView_Macro2.CurrentCell.RowIndex;
                this.dataGridView_Macro2.Rows.Insert(rowIndex + 1, 1);
                this.dataGridView_Macro2.ClearSelection();
                //totalrownumber = this.dataGridView1.Rows.Count;
            }
            catch { }
        ...
        private void Macro2_deleteRow_Click(object sender, EventArgs e)
        {
            try
            {
                Int32 rowIndex = this.dataGridView_Macro2.CurrentCell.RowIndex;
                this.dataGridView_Macro2.Rows.RemoveAt(rowIndex);
                this.dataGridView_Macro2.ClearSelection();
                //totalrownumber = this.dataGridView1.Rows.Count;
            }
            catch { }
        ...
        private void Macro2_moveUp_Click(object sender, EventArgs e)
        {

            DataGridView dgv = this.dataGridView_Macro2;
            try
            {
                Int32 totalRows = dgv.Rows.Count;
                // get index of the row for the selected cell
        ...
        private void Macro2_moveDown_Click(object sender, EventArgs e)
        {
            DataGridView dgv = this.dataGridView_Macro2;
            try
            {
